...
__global__ void standard_kernel(double a, double *out, int iters)
{
	int i;
...
__global__ void intrinsic_kernel(double a, double *out, int iters)
{
	int i;
	int tid = (blockDim.x * blockIdx.x) + threadIdx.x;
...
int main(int argc, char **argv)
{
	...
	double *d_standard_out, h_standard_out;
	gpuErrchk(cudaMalloc((void **)&d_standard_out, sizeof(double)));

	double *d_intrinsic_out, h_intrinsic_out;
	gpuErrchk(cudaMalloc((void **)&d_intrinsic_out, sizeof(double)));

	double input_value = 8181.25;
	...
	gpuErrchk(cudaMemcpy(&h_standard_out, d_standard_out, sizeof(double),
		cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(&h_intrinsic_out, d_intrinsic_out, sizeof(double),
		cudaMemcpyDeviceToHost));
	double host_value = pow(input_value, 2.0);
